...
This file intentionally mirrors the reference CLI in /memhub_cli/__main__.py so the skill can be copied standalone into Hermes/OpenClaw skill directories.
"""
...
def run_git(repo: Path, args: list[str], check: bool = False) -> subprocess.CompletedProcess[str]:
    return subprocess.run(["git", *args], cwd=str(repo), text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=check)
...
def init_repo(repo: Path, name: str = "user", role: str = "") -> None:
    ...
    ensure_repo(repo)
    for d in [".memhub/templates", "identity", "projects/memhub", "knowledge", "relations", "timeline", "inbox", "exports", "archive"]:
        (repo / d).mkdir(parents=True, exist_ok=True)
    write_yaml(repo / ".memhub/config.yaml", {
        "schema_version": SCHEMA_VERSION,
        "repo": {"id": "user_main_memhub", "owner": name},
        "sync": {"type": "git", "remote": None, "branch": "main", "auto_pull": True, "auto_push": True, "pull_strategy": "rebase", "commit_prefix": "memhub:"},
    ...
    write_yaml(repo / "identity/profile.yaml", {
        "schema_version": SCHEMA_VERSION,
        "updated_at": now_iso(),
        "profile": {"id": "user_main", "name": name, "display_name": name, "language": "zh-CN", "timezone": "Asia/Shanghai", "role": role, "company": None, "bio": "", "tags": []},
        "communication": {"default_language": "zh-CN", "preferred_style": "direct_structured", "verbosity": "medium", "likes": ["结构化输出", "直接回答", "能落地的方案"], "dislikes": ["空泛表述", "过度道歉", "没有上下文就假装知道"]},
    })
    write_yaml(repo / "identity/preferences.yaml", {"schema_version": SCHEMA_VERSION, "updated_at": now_iso(), "preferences": []})
    ...
    write_yaml(repo / "projects/_active.yaml", {"schema_version": SCHEMA_VERSION, "updated_at": now_iso(), "active_projects": [{"id": "memhub", "name": "MemHub", "priority": 1, "status": "active", "default": True, "path": "projects/memhub", "description": "跨平台 AI 记忆统一产品。"}]})
    write_yaml(repo / "projects/memhub/context.yaml", {"schema_version": SCHEMA_VERSION, "project": {"id": "memhub", "name": "MemHub", "status": "active", "description": "无服务端、Git 同步、Agent Memory Protocol。", "goals": ["让多个 AI Agent 共享同一份用户记忆。", "不依赖中心化后端。"], "constraints": ["Local-first", "Git-first", "Agent-oriented"], "current_phase": "mvp", "updated_at": now_iso()}})
    ...
    write_text(repo / ".gitignore", ".memhub/cache/\n.memhub/indexes/\n*.db\n.DS_Store\n")
...
def build_context(repo: Path, project: str | None = None, pack: str = "standard") -> str:
    ...
    lines = ["# MemHub Context Pack", "", "<!-- Auto-generated by MemHub. Treat as user-auditable memory, not absolute truth. -->", ""]
    p = profile.get("profile", {})
    c = profile.get("communication", {})
    lines += ["## User", f"- Name: {p.get('name') or ''}"]
    if p.get("role"):
    ...
    lines.append("")
    pref_items = [x for x in prefs.get("preferences", []) if x.get("status", "active") == "active"]
    pref_items.sort(key=lambda x: x.get("importance", 0), reverse=True)
    if pref_items:
        lines.append("## Important Preferences")
        for item in pref_items[:8 if pack != "brief" else 3]:
...
def remember(repo: Path, content: str, type_: str = "fact", source_client: str = "hermes", project: str | None = None) -> Path:
    ts = dt.datetime.now().strftime("%Y%m%d-%H%M%S")
    sid = short_id()
    file = repo / "inbox" / f"{ts}-{source_client}-{sid}.yaml"
    write_yaml(file, {"schema_version": SCHEMA_VERSION, "inbox_id": f"inbox_{timestamp_id()}_{sid}", "status": "pending", "created_at": now_iso(), "source": {"client": source_client}, "items": [{"id": f"mem_{timestamp_id()}_{sid}", "type": type_, "scope": "project" if project else "inbox", "status": "pending", "content": content, "suggested_target": f"projects/{project}/decisions.yaml" if type_ == "decision" and project else None, "confidence": 0.8, "importance": 0.6, "created_at": now_iso()}]})
...
def export_chatbot(repo: Path, project: str | None = None) -> Path:
    ...
    write_text(out, f"<!-- Auto-generated by MemHub. Generated at: {now_iso()} -->\n\n" + build_context(repo, project=project, pack="brief"))
...
def sync_repo(repo: Path) -> str:
    ...
    if remote:
        ...
        output += pull.stdout + pull.stderr + push.stdout + push.stderr
    else:
        output += "No git remote configured; local repository only.\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="memhub", description="MemHub Hermes Skill CLI")
    parser.add_argument("--repo", default=os.environ.get("MEMHUB_REPO", "./memhub-data"))
    ...
    p_ctx.add_argument("--pack", default="standard", choices=["brief", "standard", "full", "project"])
    ...
    p_exp.add_argument("target", choices=["chatbot"])
    p_exp.add_argument("--project", default=None)
    sub.add_parser("sync")
    args = parser.parse_args()
    repo = Path(args.repo).expanduser().resolve()
    if args.cmd == "init":
        init_repo(repo, args.name, args.role)
        print(f"Initialized MemHub repository: {repo}")
    elif args.cmd == "context":
        print(build_context(repo, args.project, args.pack), end="")
    elif args.cmd == "remember":
        print(f"Wrote inbox item: {remember(repo, args.content, args.type, args.source, args.project)}")
    elif args.cmd == "export":
        print(f"Exported chatbot context: {export_chatbot(repo, args.project)}")
    elif args.cmd == "sync":
